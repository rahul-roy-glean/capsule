...
assert_manager_gcs_mode() {
  local bucket="$1"
  local pid_file="/tmp/fc-dev/pids/firecracker-manager.pid"
  if [ ! -f "$pid_file" ]; then
    echo "FAIL: firecracker-manager pid file not found at $pid_file"
    exit 1
  fi
  ...
  if [ -z "$pid" ] || [ ! -r "/proc/$pid/cmdline" ]; then
    echo "FAIL: firecracker-manager process $pid is not readable via /proc"
    exit 1
  fi
