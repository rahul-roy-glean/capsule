...
// the capsule-thaw-agent. It creates two child cgroups under the capsule-thaw-agent's own
// cgroup:
//
//   - agent/  — the capsule-thaw-agent process itself (high CPU priority)
...
// the capsule-thaw-agent and making the sandbox uncontrollable.
...
// capsule-thaw-agent into agent/. Returns nil if cgroup v2 is not available (e.g.
...
func initCgroup() *cgroupManager {
	...
	// Move the capsule-thaw-agent into the agent cgroup.
	if err := writeCgroupFile(agentDir, "cgroup.procs", strconv.Itoa(os.Getpid())); err != nil {
		log.WithError(err).Warn("Failed to move capsule-thaw-agent into agent cgroup")
		return nil
	}
