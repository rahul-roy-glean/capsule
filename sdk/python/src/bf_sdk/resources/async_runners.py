...
from bf_sdk._http_async import AsyncHttpClient, RetryPolicy
...
_HOST_READ_RETRY_ERRORS = (BFConnectionError, BFRequestTimeoutError, BFServiceUnavailable)
_WAIT_RETRY_ERRORS = (BFConnectionError, BFNotFound, BFRateLimited, BFRequestTimeoutError, BFServiceUnavailable)
...
class AsyncRunners:
        ...
        *,
        ...
        while True:
            try:
                ...
                return resp
            except (BFRateLimited, BFServiceUnavailable, BFConnectionError, BFRequestTimeoutError) as exc:
                last_error = exc
        ...
        raise BFAllocationTimeoutError(
        ...
        *,
        ...
        while time.monotonic() < deadline:
            ...
            if result.status == "ready":
                return result
            if result.error:
                raise BFRunnerUnavailableError(
                    result.error,
                    runner_id=runner_id,
    ...
    async def allocate_ready(
        ...
        *,
        ...
        except BFOperationTimeoutError as exc:
            raise BFAllocationTimeoutError(
    ...
    async def _resolve_workload_ref(
        ...
        if self._layered_configs is None:
            ...
            raise BFNotFound("This runner client cannot resolve workload references without layered config support.")
