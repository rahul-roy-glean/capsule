...
class Runners:
    ...
    def connect(self, runner_id: str | None = None, *, session_id: str | None = None) -> ConnectResult:
        ...
        if session_id:
            body["session_id"] = session_id
        data = self._http.post("/api/v1/runners/connect", json_body=body)
        result = ConnectResult.model_validate(data)
        if result.host_address:
            self._host_cache[result.runner_id] = result.host_address
        return result

    def attach_session(self, session_id: str) -> RunnerSession:
        result = self.connect(session_id=session_id)
    ...
    def fork(self, runner_id: str | None = None, *, session_id: str | None = None) -> ForkSessionResponse:
        if not runner_id and not session_id:
            raise ValueError("runner_id or session_id is required")
        body: dict[str, str] = {}
        if runner_id:
        ...
        if session_id:
            body["session_id"] = session_id
        data = self._http.post("/api/v1/sessions/fork", json_body=body)
