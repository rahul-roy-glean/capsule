...
func TestTokenExchangeProvider_InjectCredentials_CustomHeader(t *testing.T) {
	p := &tokenExchangeProvider{
		header: "X-Custom-Auth",
		prefix: "Token ",
	}
	p.token = "custom-token"
	p.expiresAt = time.Now().Add(1 * time.Hour)
	...
	if got != "Token custom-token" {
		t.Fatalf("expected 'Token custom-token', got %q", got)
	}
}
...
func TestTokenExchangeProvider_Defaults(t *testing.T) {
	cfg := authproxy.ProviderConfig{
		Type:  "token-exchange",
		Hosts: []string{"example.com"},
		Config: map[string]string{
	...
	provider, err := newTokenExchangeProvider(cfg)
	if err != nil {
		t.Fatalf("unexpected error: %v", err)
	}

	p := provider.(*tokenExchangeProvider)
	if p.grantType != "urn:ietf:params:oauth:grant-type:jwt-bearer" {
		t.Errorf("expected default grant type, got %q", p.grantType)
	}
	if p.header != "Authorization" {
		t.Errorf("expected default header 'Authorization', got %q", p.header)
	}
	if p.prefix != "Bearer " {
		t.Errorf("expected default prefix 'Bearer ', got %q", p.prefix)
	}
}
